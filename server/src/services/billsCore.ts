/**
 * Bills: the pure engine.
 *
 * Split out from billsService.ts so it can run in two places — the server,
 * where charges come from SQLite, and the hosted demo, which runs this exact
 * code in a browser with no database at all. billsService.ts keeps the Prisma
 * queries and delegates here, so there is one implementation of the maths.
 */
